...
PR = {
    ...
    "Vacaciones":       "54575367-4d4e-4d0c-a0fd-8be7d33db21e",
    "Impuestos":        "224cdb40f1f749c7b5d6e165ad31170d",
...
# Mapeo para limpiar emojis del texto seleccionado
def limpiar_emoji(texto):
    for grupo in list(GRUPOS_CAT.keys()) + [r[0] for r in MENU_PRESUPUESTO+MENU_GRUPOS for _ in [0]]:
        pass
    # Quitar el emoji prefix (primeros chars hasta el espacio)
    t = texto.strip()
    partes = t.split(" ", 1)
...
def grupo_sin_emoji(texto):
    """Extrae el nombre del grupo sin emoji"""
    t=texto.strip()
    for grp in GRUPOS_CAT:
        if t==grp: return grp
        # Comparar sin emoji
        grp_limpio=" ".join(grp.split()[1:]) if len(grp.split())>1 else grp
        t_limpio=" ".join(t.split()[1:]) if len(t.split())>1 else t
        if t_limpio==grp_limpio: return grp
    return t

def presu_sin_emoji(texto):
    """Extrae el nombre del presupuesto sin emoji"""
    t=texto.strip()
    partes=t.split(" ",1)
...
async def confirmar_cat(update,context):
    ...
    grp=grupo_sin_emoji(update.message.text.strip())
    subcats=GRUPOS_CAT.get(grp,[grp])
    gasto["subcategoria"]=subcats[0]
    gasto["presupuesto"]=grp.split(" ",1)[-1] if " " in grp else grp
...
async def cmd_corregir(update,context):
    ...
    await update.message.reply_text(texto,reply_markup=ReplyKeyboardMarkup([[f"{i+1}"] for i in range(len(ultimos))],one_time_keyboard=True,resize_keyboard=True))
...
async def corregir_que(update,context):
    ...
    context.user_data["que_corregir"]=eleccion
    gasto=context.user_data.get("gasto_corregir")
    if "Presupuesto" in eleccion and "Subcategoría" not in eleccion and "Ambas" not in eleccion:
...
async def corregir_cat_grp(update,context):
    grp=grupo_sin_emoji(update.message.text.strip())
    context.user_data["grupo_elegido"]=grp
    subcats=GRUPOS_CAT.get(grp,[grp])
    if len(subcats)==1:
        context.user_data["nueva_sub"]=subcats[0]
        que=context.user_data.get("que_corregir","")
        if "Ambas" in que or "ambas" in que.lower():
            await update.message.reply_text(f"Subcategoria: {subcats[0]}\n\n💰 Paso 2: Elige el presupuesto:",reply_markup=ReplyKeyboardMarkup(MENU_PRESUPUESTO,one_time_keyboard=True,resize_keyboard=True))
            return CORREGIR_PRESU
        else:
            return await aplicar_correccion(update,context,sub=subcats[0])
    menu=[[s] for s in subcats]
    await update.message.reply_text(f"🏷️ Paso 2: Elige la subcategoria de {grp.split(' ',1)[-1]}:",reply_markup=ReplyKeyboardMarkup(menu,one_time_keyboard=True,resize_keyboard=True))
    return CORREGIR_SUBCAT

async def corregir_subcat(update,context):
    context.user_data["nueva_sub"]=update.message.text.strip()
    que=context.user_data.get("que_corregir","")
    if "Ambas" in que:
        await update.message.reply_text("💰 Paso 3: Elige el presupuesto:",reply_markup=ReplyKeyboardMarkup(MENU_PRESUPUESTO,one_time_keyboard=True,resize_keyboard=True))
        return CORREGIR_PRESU
    else:
        return await aplicar_correccion(update,context,sub=context.user_data["nueva_sub"])

async def corregir_presu(update,context):
    nuevo_pre=presu_sin_emoji(update.message.text.strip())
    return await aplicar_correccion(update,context,pre=nuevo_pre)

async def aplicar_correccion(update,context,sub=None,pre=None):
    gasto=context.user_data.get("gasto_corregir")
    nueva_sub=sub or context.user_data.get("nueva_sub")
    nuevo_pre=pre or context.user_data.get("nuevo_pre")
    if not gasto:
        await update.message.reply_text("Error.",reply_markup=ReplyKeyboardRemove()); return ConversationHandler.END
...
def main():
    ...
    print("Bot corriendo con menu completo y emojis...")
